...
;(function(window, factory) {
    ...
    function jeDatePick(elem, options){
        var config = {
            ...
            initDate:[{MM:"+3"},true],                 //初始化时间，加减 天 时 分
    ...
    jet.extend(jeDatePick.prototype,{
        ...
        renderDate : function () {
            ...
            // 按钮 置灰 判断
            var mins = jet.reMatch(that.minDate), maxs = jet.reMatch(that.maxDate);
            var curDay = that.$data.daytit[0];
            // 判断 年 按钮是否可点击
            if(curDay['YYYY']>=maxs[0]){
                $(that.dateCell).find('.ynext').addClass('disabled');
            }else if(curDay['YYYY']<=mins[0]){
                $(that.dateCell).find('.yprev').addClass('disabled');
            }
            // 判断 月份按钮 是否可点击
            
            if(curDay['YYYY']>maxs[0]||curDay['YYYY']<mins[0]){
                return false;
            }else if(mins[0]==curDay['YYYY']){
                if(curDay['MM']==mins[1]){
                    $(that.dateCell).find('.mprev').addClass('disabled');
                }
            }else if(maxs[0]==curDay['YYYY']){
                if(curDay['MM']==maxs[1]){
                    $(that.dateCell).find('.mnext').addClass('disabled');
                }
            }
            //自定义主题色
        ...
        dateTemplate : function() {
            ...
            var lyPrev = opts.showYearBtn?'<em class="yearprev yprev jedatefont" @on="yearBtn(lprev,'+aowArr[0]+')">&#xed6c2;</em>':'',
                lyNext = opts.showYearBtn?'<em class="yearnext ynext jedatefont" on="yearBtn(lnext,'+aowArr[2]+')">&#xed6c5;</em>':'',
                ryPrev = opts.showYearBtn?'<em class="yearprev yprev jedatefont" on="yearBtn(rprev,'+aowArr[3]+')">&#xed6c2;</em>':'',
                ryNext = opts.showYearBtn?'<em class="yearnext ynext jedatefont" @on="yearBtn(rnext,'+aowArr[1]+')">&#xed6c5;</em>':'',
        ...
        methodEventBind : function() {
            ...
            jet.extend(that,{
                yearBtn:function (type,val) {
                    ...
                    var addval , nextM ;
                    var storeDataInit=false;
                    
                    if(yarr[0]>maxs[0]||yarr[0]<mins[0]){
                        return false
                    }else if(yarr[0]==maxs[0]){
                        addval = [{
                            'YYYY':maxs[0],
                            'MM':maxs[1],
                            'DD':maxs[2],
                        ...
                        that.setValue([addval[0]],opts.format,true);
                        nextM = jet.nextMonth(addval[0].YYYY,addval[0].MM);
                        storeDataInit = true;
                        // that.storeData(addval[0], nextM);
                        /*inVal = [{
                            YYYY:addval.YYYY, MM:jet.digit(addval.MM), DD:jet.digit(addval.DD) , 
                            hh:jet.digit(addval.hh), mm:jet.digit(addval.mm), ss:jet.digit(addval.ss) 
                        }];
                        that.setValue([inVal[0]],opts.format,true);*/
                    }else if(yarr[0]==mins[0]){
                        addval = [{
                            'YYYY':mins[0],
                            'MM':mins[1],
                            'DD':mins[2],
                    ...
                    storeDataInit?that.storeData(addval[0], nextM):that.storeData(exarr[0],exarr[1]);
                ...
                monthBtn:function (type,val) { 
                    ...
                    if(PNYM['y']>maxs[0]||PNYM['y']<mins[0]){
                        return false;
                    }else if(mins[0]==PNYM['y']){
                        if(PNYM['m']<mins[1]){
                            $(that.dateCell).find('.'+type).addClass('disabled');
                            return false
                        }
                    }else if(maxs[0]==PNYM['y']){
                        if(PNYM['m']>maxs[1]){
                            $(that.dateCell).find('.'+type).addClass('disabled');
                            return false
                        }
        ...
        eachDays  : function (yd,md,ds,idx) {
            ...
            //自定义 mark 已售馨
            var setScwMark = function (my, mm, md) {
                var scwMarks = opts.scwMarks, contains = function(arr, obj) {
                    var clen = arr.length;
                    while (clen--) {  
                        if (arr[clen].planDate === obj) {
                            index = clen; 
                            return true;
                        } 
                    }
                    return false;
                },isArr = jet.isType(scwMarks,"array"),index;  
                return isArr && scwMarks.length > 0 && contains(scwMarks, my + "-" + jet.digit(mm) + "-" + jet.digit(md)) ? '<i class="scwMarks">￥'+scwMarks[index].tuniuPrice+'</i>' :"";
            };
            //是否显示节日
            ...
            for (var p = prevDaysNum - firstWeek + 1; p <= prevDaysNum; p++, count++) {
                var pmark = setMark(prevM.y,prevM.m,p), pScwMark = setScwMark(prevM.y,prevM.m,p), pcls = dateLimit(prevM.y, prevM.m, p, false) ? "disabled" : "other";
                pcls = regExpDate(p,pcls);
                // geiscwmark 添加 disabled
                /*if(pScwMark!=""&&pcls.indexOf('disabled')<0){
                   pcls = +" disabled" 
                }*/
                daysArr.push({style:pcls,ymd:prevM.y+'-'+jet.digit(prevM.m)+'-'+jet.digit(p),day:(isfestival(prevM.y,prevM.m,p) + pmark + pScwMark)});
            }
            //本月的天数
            for(var b = 1; b <= daysNum; b++, count++){
                var bmark = setMark(yd,md,b), bcls = "";
                var bScwMark = setScwMark(yd,md,b);
                ...
                // geiscwmark 添加 disabled
               /* if(bScwMark!=""&&bcls.indexOf('disabled')<0){
                   bcls  = bcls + " disabled" 
                }*/
                daysArr.push({style:"normal"+bcls,ymd:yd+'-'+jet.digit(md)+'-'+jet.digit(b),day:(isfestival(yd,md,b) + bmark + bScwMark)});
            }
            //下一月开始天数
            for(var n = 1, nlen = 42 - count; n <= nlen; n++){
                var nmark = setMark(nextM.y,nextM.m,n);
                var nScwMark = setScwMark(nextM.y,nextM.m,n);
                var ncls = dateLimit(nextM.y, nextM.m, n, false) ? "disabled" : "other";
                ncls = regExpDate(n,ncls);
                if(nScwMark!=""&&ncls.indexOf('disabled')<0){
                   ncls  = ncls + " disabled" 
                }
                daysArr.push({style:ncls,ymd:nextM.y+'-'+jet.digit(nextM.m)+'-'+jet.digit(n),day:(isfestival(nextM.y,nextM.m,n) + nmark + nScwMark)});
            }
            //将星期与日期拼接起来
            return daysArr;  
        },
